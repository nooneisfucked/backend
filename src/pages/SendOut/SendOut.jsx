...
const SendOut = () => {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/smtp/send/status');
  ...
  const handleToggle = async () => {
    ...
    try {
      const endpoint = newState ? 'http://localhost:3000/api/smtp/send/start' : 'http://localhost:3000/api/smtp/send/stop';
